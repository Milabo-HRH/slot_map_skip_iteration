...
```
ordered_slot_map<std::string> strings;
ordered_slot_map<int> numbers;
ordered_slot_map<int>::key numKey =  numbers.emplace(3);
...
*/
template <typename T, typename TKeyType = slot_map_key64<T>, size_t PAGESIZE = 4096, size_t MINFREEINDICES = 64> class ordered_slot_map
{
  public:
    using key = TKeyType;
  ...
  private:
    ...
    struct Meta
    {
        ...
        index_t next;      // index of the next active slot
        index_t prev;      // index of the previous active slot
    };
    ...
    void copyFrom(const ordered_slot_map& other)
    {
        reset();
  ...
  public:
    ...
    */
    void swap(ordered_slot_map& other) noexcept
    {
    ...
    // copy constructor
    ordered_slot_map(const ordered_slot_map& other)
        : numItems(0)
        , maxValidIndex(0)
        , tailIndex(0)
        , headIndex(0)
    ...
    // copy assignment
    ordered_slot_map& operator=(const ordered_slot_map& other)
    {
        copyFrom(other);
        return *this;
    }
    ...
    // move constructor
    ordered_slot_map(ordered_slot_map&& other) noexcept
        : numItems(other.numItems)
        , maxValidIndex(other.maxValidIndex)
        , tailIndex(other.tailIndex)
        , headIndex(other.headIndex)
    ...
    // move asignment
    ordered_slot_map& operator=(ordered_slot_map&& other) noexcept
    {
        // reset and swap
        reset();
  ...
  public:
    ...
    // for(const auto& kv : ordered_slot_map.items()) {} // key/value
    // for(const auto& v : ordered_slot_map) {} // values only
    ...
    class const_values_iterator
    {
      ...
      public:
        explicit const_values_iterator(const ordered_slot_map* _slotMap, size_type index) noexcept
            : slotMap(_slotMap)
            , currentIndex(index)
        {
        }
        ...
        const_values_iterator& operator++() noexcept
        {
            ...
            {
                currentIndex = slotMap->getMaxValidIndex() + static_cast<size_type>(1);
            }
            return *this;
        }
      ...
      private:
        const ordered_slot_map* slotMap;
    ...
    class const_kv_iterator
    {
      ...
      public:
        explicit const_kv_iterator(const ordered_slot_map* _slotMap, size_type index) noexcept
            : slotMap(_slotMap)
            , currentIndex(index)
    ...
    class Items
    {
      ...
      public:
        explicit Items(const ordered_slot_map* _slotMap) noexcept
            : slotMap(_slotMap)
        {
        }
        ...
        {
            if (slotMap->pages.empty() || slotMap->headIndex==0) return end();
...
using slot_map32 = ordered_slot_map<T, dod::slot_map_key32<T>, PAGESIZE, MINFREEINDICES>;

template <class T, size_t PAGESIZE = 4096, size_t MINFREEINDICES = 64>
using slot_map64 = ordered_slot_map<T, dod::slot_map_key64<T>, PAGESIZE, MINFREEINDICES>;
