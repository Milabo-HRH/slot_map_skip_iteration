...
```
slot_map_low_complexity<std::string> strings;
slot_map_low_complexity<int> numbers;
slot_map_low_complexity<int>::key numKey =  numbers.emplace(3);
...
*/
template <typename T, typename TKeyType = slot_map_key64<T>, size_t PAGESIZE = 4096, size_t MINFREEINDICES = 64> class slot_map_low_complexity
{
  public:
    using key = TKeyType;
...
//        uint8_t tombstone; // note: we only need 1 bit for tombstone marker
    ...
    struct Page
    {
        ...
        std::array<index_t, kPageSize> jump;
...
//        SLOT_MAP_ASSERT(m.tombstone == 0);
        SLOT_MAP_ASSERT(getJump(index) == 0);
    ...
    bool isTombstone(size_type index) const noexcept
    {
        ...
        SLOT_MAP_ASSERT(addr.page < pages.size());
        const Page& page = pages[addr.page];
        return page.jump[addr.index] != 0;
    }

    index_t getJump(size_type index) const noexcept
    {
        if (index > getMaxValidIndex()||index<0)
        {
            return 0;
        }
        PageAddr addr = getAddrFromIndex(index);
        SLOT_MAP_ASSERT(addr.page < pages.size());
        const Page& page = pages[addr.page];
        return page.jump[addr.index];
    }

    bool isBetweenCycledSlots(size_type index) const noexcept
    {
        if (index==0 || index==maxValidIndex )
        {
            return false;
        }
        else if (isTombstone(index-1) && isTombstone(index+1))
        {
            return true;
        }
        return false;
    ...
    void copyFrom(const slot_map_low_complexity& other)
    {
        reset();
        ...
        SLOT_MAP_ASSERT(edgeIndicies.empty());
        SLOT_MAP_ASSERT(innerIndicies.empty());
        ...
        {
            if (const Page& otherPage = other.pages[pageIndex]; otherPage.meta)
            {
    ...
    void setJump(size_type index, index_t jump) noexcept
    {
        PageAddr addr = getAddrFromIndex(index);
        SLOT_MAP_ASSERT(addr.page < pages.size());
        Page& page = pages[addr.page];
        page.jump[addr.index] = jump;
    }

    void callDtors()
    {
        size_type numItemsDestroyed = 0;
        size_type elementIndex = 0;
        if(pages.empty())
        {
            return;
        }
        while(elementIndex<=maxValidIndex) {
            if(size_type j = getJump(elementIndex); j!=0)
            {
                elementIndex += j;
                continue;
            }
    ...
    template <typename index_t> void handleBetweenCycled(index_t index)
    {
        slot_map_low_complexity::index_t jumpBg = getJump(index - 1);
        slot_map_low_complexity::index_t jumpEn = getJump(index + 1);
        // m.jump = jumpBg + jumpEn + 1;
        setJump(index, jumpBg + jumpEn + 1);
        setJump(index + jumpEn, jumpBg + jumpEn + 1);
        setJump(index - jumpBg, jumpBg + jumpEn + 1);
    ...
    template <bool VERSION_CHECK> EraseResult eraseImpl(key k)
    {
        ...
        Meta& m = getMetaByAddr(addr);
        if (pages[addr.page].jump[addr.index] != 0)
        {
            return EraseResult::NotFound;
        }
  ...
  public:
    slot_map_low_complexity()
        : numItems(0)
        , maxValidIndex(0)
    {
    }
    ~slot_map_low_complexity() { callDtors(); }
    ...
    */
    void reset()
    {
        ...
        if (!edgeIndicies.empty())
        {
            std::unordered_map<index_t, key, std::hash<index_t>, std::equal_to<index_t>, stl::Allocator<std::pair<const index_t, key>>> tmpFreeIndices;
            edgeIndicies.swap(tmpFreeIndices);
        }

        if (!innerIndicies.empty())
        {
            std::unordered_map<index_t, key, std::hash<index_t>, std::equal_to<index_t>, stl::Allocator<std::pair<const index_t, key>>> tmpFreeIndices;
            innerIndicies.swap(tmpFreeIndices);
        }
    }
...
private:
    ...
    */
    void handleRightEdge(index_t index, index_t jump)
    {

        setJump(index - jump+1, jump-1);
        setJump(index-1, jump-1);
        setJump(index, 0);
    ...
    */
    void handleLeftEdge(index_t index, index_t jump)
    {
...
public:
    ...
    */
    template <class... Args> key emplace(Args&&... args)
    {
        ...
        if (static_cast<size_type>(innerIndicies.size() + edgeIndicies.size()) > kMinFreeIndices && !edgeIndicies.empty())
        {
            ...
            SLOT_MAP_ASSERT(!isBetweenCycledSlots(index));
            ...
            if(getJump(index-1) )
            {
                ...
                handleRightEdge(index, getJump(index));
            }
            else if(getJump(index+1))
            {
                // if the slot is at the left edge of a contiguous sequence of recycled slots
                handleLeftEdge(index, getJump(index));
            } else
            {
                setJump(index, 0);
            }
    ...
    */
    void swap(slot_map_low_complexity& other) noexcept
    {
        pages.swap(other.pages);
//        freeIndices.swap(other.freeIndices);
        edgeIndicies.swap(other.edgeIndicies);
        innerIndicies.swap(other.innerIndicies);
        std::swap(numItems, other.numItems);
        std::swap(maxValidIndex, other.maxValidIndex);
    }

    // copy constructor
    slot_map_low_complexity(const slot_map_low_complexity& other)
        : numItems(0)
        , maxValidIndex(0)
    {
        copyFrom(other);
    ...
    // copy assignment
    slot_map_low_complexity& operator=(const slot_map_low_complexity& other)
    {
        copyFrom(other);
        return *this;
    }
    ...
    // move constructor
    slot_map_low_complexity(slot_map_low_complexity&& other) noexcept
        : numItems(other.numItems)
        , maxValidIndex(other.maxValidIndex)
    {
        std::swap(pages, other.pages);
//        std::swap(freeIndices, other.freeIndices);
        std::swap(edgeIndicies, other.edgeIndicies);
        std::swap(innerIndicies, other.innerIndicies);
        other.numItems = 0;
        other.maxValidIndex = 0;
    }

    // move asignment
    slot_map_low_complexity& operator=(slot_map_low_complexity&& other) noexcept
    {
        // reset and swap
        reset();
...
//        freeIndices.swap(other.freeIndices);
    ...
    Stats debug_stats() const noexcept
    {
        ...
        for (size_t pageIndex = 0; pageIndex < pages.size(); pageIndex++)
        {
            ...
            for (size_type elementIndex = 0; elementIndex < page.numUsedElements; elementIndex++)
            {
                ...
                else if (pages[addr.page].jump[addr.index] != 0)
                {
  ...
  public:
    ...
    // for(const auto& kv : slot_map_low_complexity.items()) {} // key/value
    // for(const auto& v : slot_map_low_complexity) {} // values only
    ...
    class const_values_iterator
    {
      ...
      public:
        explicit const_values_iterator(const slot_map_low_complexity* _slotMap, size_type index) noexcept
            : slotMap(_slotMap)
            , currentIndex(index)
        {
        }
        ...
        const_values_iterator& operator++() noexcept
        {
            ...
            while (currentIndex <= slotMap->getMaxValidIndex() && slotMap->getJump(currentIndex))
            {
                SLOT_MAP_ASSERT(currentIndex <= slotMap->getMaxValidIndex());
                currentIndex += slotMap->getJump(currentIndex);
            }
            return *this;
        }
    ...
    class const_kv_iterator
    {
      ...
      public:
        explicit const_kv_iterator(const slot_map_low_complexity* _slotMap, size_type index) noexcept
            : slotMap(_slotMap)
            , currentIndex(index)
    ...
    class Items
    {
      ...
      public:
        explicit Items(const slot_map_low_complexity* _slotMap) noexcept
            : slotMap(_slotMap)
        {
        }
...
using slot_map32 = slot_map_low_complexity<T, dod::slot_map_key32<T>, PAGESIZE, MINFREEINDICES>;

template <class T, size_t PAGESIZE = 4096, size_t MINFREEINDICES = 64>
using slot_map64 = slot_map_low_complexity<T, dod::slot_map_key64<T>, PAGESIZE, MINFREEINDICES>;
